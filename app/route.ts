...
const REAL_TOKEN_QUANTITIES = {
  cvx: 142000,   // vlCVX position
  aero: 220000,   // Static fallback only — real fallback is the growing estimate below
  rsup: 166000,  // RSUP token count
  yb: 300000      // YB token count
};

// --------------------------------------------------------------------------
// AERO locked-token weekly rebase
// Your locked AERO position inflates every Wednesday ~8:00 PM ET as veAERO rebase
// rewards land. Confirmed snapshot: 220,710 AERO as of 2026-08-07, growing 2.38%/week.
// If the live Base RPC balance fetch below succeeds, that real on-chain number is used
// instead — this is only the fallback estimate for when it doesn't (e.g. the balance is
// held in a lock/NFT position rather than a plain wallet balance the RPC call can read).
// Update AERO_LOCK_SNAPSHOT and AERO_WEEKLY_REBASE_RATE here whenever you get a fresh
// on-chain reading, so the estimate doesn't drift too far from reality.
// --------------------------------------------------------------------------
const AERO_LOCK_SNAPSHOT_QTY = 220710;
const AERO_LOCK_SNAPSHOT_MS = Date.UTC(2026, 7, 7, 12, 0, 0); // 2026-08-07 (month is 0-indexed)
const AERO_WEEKLY_REBASE_RATE = 0.0238; // 2.38%/week, as reported 2026-08-07

// Counts Wednesdays (UTC calendar day) strictly between two timestamps — used to estimate
// how many weekly rebases have occurred since the snapshot above. This counts by calendar
// day rather than the exact 8PM ET cutoff, which is close enough for a portfolio estimate.
function countWednesdaysBetween(fromMs: number, toMs: number): number {
  if (toMs <= fromMs) return 0;
  const oneDay = 24 * 60 * 60 * 1000;
  let count = 0;
  let cursor = Math.floor(fromMs / oneDay) * oneDay + oneDay;
  while (cursor <= toMs) {
    if (new Date(cursor).getUTCDay() === 3) count++;
    cursor += oneDay;
  }
  return count;
}

function getEstimatedAeroQty(): number {
  const weeksElapsed = countWednesdaysBetween(AERO_LOCK_SNAPSHOT_MS, Date.now());
  return AERO_LOCK_SNAPSHOT_QTY * Math.pow(1 + AERO_WEEKLY_REBASE_RATE, weeksElapsed);
}
...
export async function GET() {
  ...
  const aeroFallbackQty = DEMO_MODE ? activeQuantities.aero : getEstimatedAeroQty();
  ...
  const [cvxPrice, aeroPrice, rsupPrice, ybPrice, liveAeroQty] = await Promise.all([
    ...
    getBaseAeroBalance(WALLET_ADDRESS, aeroFallbackQty)
  ]);
  ...
  return NextResponse.json({
    ...
    assets: {
      ...
      rsup: {
        ...
        quantity: activeQuantities.rsup,
