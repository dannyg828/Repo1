        ...
        Object.entries(data.assets).map(([key, asset]: [string, any]) =>
          el(
            ...
            el(
              ...
              el('div', null, el('span', { className: c.textMuted }, 'Principal Value: '), el('span', { className: `font-semibold ${c.textMain}` }, fmt(asset.principalUSD))),
    ...
    content = el(
      ...
      el(
        ...
        el(
          'div',
          null,
          el('h2', { className: `text-xl font-bold ${c.textMain}` }, `${selectedAsset.name} (${selectedAsset.symbol})`),
          el('p', { className: `text-xs ${c.textMuted} mt-1` }, `Revenue Mechanism: ${selectedAsset.revenueSource}`)
        ),
        el(
          'div',
          { className: 'flex gap-4 text-xs' },
          el('div', { className: `${c.subCard} px-3 py-2 rounded-lg` }, el('span', { className: `${c.textMuted} block` }, 'Position Value'), el('span', { className: `font-bold ${c.textMain}` }, fmt(selectedAsset.principalUSD))),
          el('div', { className: `${c.subCard} px-3 py-2 rounded-lg` }, el('span', { className: `${c.textMuted} block` }, 'Monthly Yield'), el('span', { className: 'font-bold text-emerald-500' }, isPrivacy ? '$••••••' : `$${selectedAsset.monthlyCashFlowUSD.toLocaleString()}/mo`)),
          el('div', { className: `${c.subCard} px-3 py-2 rounded-lg` }, el('span', { className: `${c.textMuted} block` }, 'Current APR'), el('span', { className: 'font-bold text-purple-500' }, selectedAsset.apr))
        )
      ),
