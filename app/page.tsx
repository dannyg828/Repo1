...
export default function Dashboard() {
  ...
  const [assetsView, setAssetsView] = useState<'positions' | 'simulation'>('positions');
  const [simQty, setSimQty] = useState<Record<string, string>>({});
  ...
  if (activeTab === 'overview') {
          ...
          el(
            ...
            el(
              ...
              el(
                ...
                el('span', { className: `${c.textMuted} ml-1` }, isPrivacy ? '(••••)' : `(${Math.round(asset.quantity).toLocaleString()} ${key === 'cvx' ? 'CVX' : asset.symbol})`)
              ),
    ...
    const assetEntries = Object.entries(data.assets) as [string, any][];

    const subNav = el(
      'div',
      { className: 'flex gap-2' },
      ['positions', 'simulation'].map((v) =>
        el(
          'button',
          {
            key: v,
            onClick: () => setAssetsView(v as any),
          ...
          v === 'positions' ? 'Positions' : 'Simulation'
        )
      )
    );

    const positionsContent = el(
      'div',
      { className: 'space-y-10' },
      assetEntries.map(([key, asset]) => renderAssetDetail(key, asset))
    );

    // Simulation: lets you type a hypothetical token amount for any asset and see an
    // approximate yearly revenue at today's price and APR. Purely a client-side
    // calculator — nothing here is submitted or saved anywhere.
    const simRows = assetEntries.map(([key, asset]) => {
      const displaySymbol = key === 'cvx' ? 'CVX' : asset.symbol;
      const price = asset.quantity > 0 ? asset.principalUSD / asset.quantity : 0;
      const aprFraction = (parseFloat(asset.apr) || 0) / 100;
      const qtyInput = simQty[key] ?? '';
      const qtyNum = parseFloat(qtyInput) || 0;
      const estYearly = qtyNum * price * aprFraction;
      return { key, asset, displaySymbol, qtyInput, estYearly };
    });
    const totalEst = simRows.reduce((sum, r) => sum + r.estYearly, 0);

    const simulationContent = el(
      'div',
      { className: `${c.card} p-5 rounded-xl space-y-4` },
      el('h3', { className: `text-xs font-semibold ${c.textMuted} uppercase tracking-wider` }, 'Revenue Simulation'),
      el(
        'p',
        { className: `text-xs ${c.textMuted}` },
        "Enter a hypothetical token amount for any asset to estimate its approximate yearly revenue, using today's price and APR."
      ),
      el(
        'div',
        { className: 'space-y-3' },
        simRows.map((r) =>
          el(
            'div',
            { key: r.key, className: `flex flex-col md:flex-row md:items-center gap-3 border-t ${c.border} pt-3 first:border-t-0 first:pt-0` },
            el(
              'div',
              { className: 'md:w-40 flex-shrink-0' },
              el('span', { className: `block font-semibold ${c.textMain} text-sm` }, r.asset.name),
              el('span', { className: `text-xs ${c.textMuted}` }, r.displaySymbol)
            ),
            el('input', {
              type: 'number',
              min: '0',
              placeholder: `${r.displaySymbol} amount`,
              value: r.qtyInput,
              onChange: (e: any) => setSimQty((prev) => ({ ...prev, [r.key]: e.target.value })),
              className: `flex-1 border ${c.border} rounded-lg px-3 py-2 text-sm ${c.subCard} ${c.textMain} outline-none focus:border-emerald-500`
            }),
            el(
              'div',
              { className: 'text-right md:w-40 flex-shrink-0' },
              el('span', { className: `block text-xs ${c.textMuted}` }, 'Est. yearly revenue'),
              el('span', { className: 'font-bold text-emerald-500' }, isPrivacy ? '$••••••' : `$${Math.round(r.estYearly).toLocaleString()}`)
            )
          )
        )
      ),
      el(
        'div',
        { className: `flex items-center justify-between border-t ${c.border} pt-4` },
        el('span', { className: `text-sm font-semibold ${c.textMain}` }, 'Total estimated yearly revenue'),
        el('span', { className: 'text-lg font-bold text-emerald-500' }, isPrivacy ? '$••••••' : `$${Math.round(totalEst).toLocaleString()}`)
      )
    );
    ...
    content = el(
      ...
      { className: 'space-y-6' },
      subNav,
      assetsView === 'positions' ? positionsContent : simulationContent
    );
