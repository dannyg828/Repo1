...
import { OrderBookApi, OrderSigningUtils, SupportedChainId, setGlobalAdapter, OrderQuoteSideKindSell } from '@cowprotocol/cow-sdk';
...
export async function swapToUSDC(provider: BrowserProvider, sellTokenAddress: string, sellAmountWei: bigint): Promise<SwapResult> {
  ...
  const orderBookApi = new OrderBookApi({ chainId: SupportedChainId.MAINNET, env: 'prod' });
  ...
  const signingResult = await OrderSigningUtils.signOrder(quote as any, SupportedChainId.MAINNET, signer);
